...
function performAction(e) {
    ...
    const timestamp = (new Date(feelings).getTime()) / 1000;
    const daysLeft = Math.round((timestamp - timestampNow) / 86400);

    console.log(feelings, timestampNow, timestamp, daysLeft);
    //We get the weather data and THEN create a POST from that data
    getReport(baseURL, getZip, apiUsername)
        //New Syntax!
        .then(function (data) {
            postData("/add", { city: data.geonames[0].lat, date: data.geonames[0].lng, temp: data.geonames[0].countryName, feelings: daysLeft });
...
const getReport = async (baseURL, zip, key) => {
    const res = await fetch(baseURL + "/searchJSON?q=" + zip + "&maxRows=10&username=" + key);
    try {
